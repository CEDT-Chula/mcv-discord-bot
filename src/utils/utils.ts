...
async function sendToAdminIfEnv(message: string) {
...
export async function updateCourses() {
  let response = await fetch(`https://www.mycourseville.com/`, {
    ...
    await sendToAdminIfEnv("Error fetching, Might be rate limited or server is down")
  })
  ...
  if (response.status != 200) {
    ...
    return await sendToAdminIfEnv("Error fetching, Might be rate limited or server is down")
  }
  if (hasEncounteredError) {
    await sendToAdminIfEnv("server back to normal")
  }
